...
public class Utilities {
     ...
     */
    public static boolean saveNote(Context context, LihatCatat note) {
     ...
     */
    static ArrayList<LihatCatat> getAllSavedNotes(Context context) {
        ArrayList<LihatCatat> notes = new ArrayList<>();
        ...
        for (int i = 0; i < noteFiles.size(); i++) {
            try{
                ...
                notes.add((LihatCatat) ois.readObject());
                fis.close();
                ois.close();
     ...
     */
    public static LihatCatat getNoteByFileName(Context context, String fileName) {
        ...
        if(file.exists() && !file.isDirectory()) { //check if file actually exist
            ...
            try { //load the file
                ...
                ois = new ObjectInputStream(fis);
                LihatCatat note = (LihatCatat) ois.readObject();
                fis.close();
                ois.close();
